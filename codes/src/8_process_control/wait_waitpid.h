...
/*
 * \brief My_wartpid : 包装了 waitpid 函数
 * \param pid : 传递给 waitpid 函数的 pid 函数
 * \param staloc : 传递给 waitpid 函数的 staloc 函数
 * \param options : 传递给 waitpid 函数的 options 函数
 ...
 *
 *  主要是进程在执行`waitpid`调用而阻塞期间捕获到一个信号，则该系统调用就会被中断而不再继续执行
...
/*
 * \brief testJ_wait_waitpid : 测试 wait 、waitpid 函数
 */
