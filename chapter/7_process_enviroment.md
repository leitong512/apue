...
2.
当程序处于交互方式运行时，标准输出通常处于行缓冲模式，所以当输出换行符时，上次的结果才被真正的输出。
...
4. 
    当 C 程序解引用一个空指针出错时，执行该程序的进程将终止。可以利用这个方式终止进程
...
6.
    ...
    因此数组的初始值为 0 ，但是 ISO C 不保证 0 值和空指针的值相等。

7.
    堆和栈的大小根据程序运行时的条件不同，所占空间也是不同的。
    只有通过`exec`函数执行一个程序时，才会分配堆和栈。

10.
    不正确。if 语句里面的 val 是一个自动变量，当离开 if 的作用域，这个变量不存在。 
